# Fracktal Works F&A

> Generated by DOE Framework Agent Workspace Generator

**Type:** Finance & Accounting Workflow Agent

Generates comprehensive workflow charts for all F&A department processes at Fracktal Works Private Limited — including Accounts Payable, Accounts Receivable, Month-End Close, Payroll, GST Compliance, Payment Approval Policy, and more. Output: professionally formatted Excel workbooks with swim-lane flowcharts, RACI matrices, approval matrices, and KPI dashboards.

## 🚀 Instant Start

**Just double-click:** `fracktal-works-f&a.code-workspace`

VS Code will automatically:
1. Open the workspace
2. Prompt to trust the folder (click **Yes**)
3. Run setup (creates venv, installs dependencies)
4. Prompt to install recommended extensions

Then select **"Fracktal Works F&A"** from the Copilot Chat agent dropdown and start working!

## Alternative: Manual Setup

**Windows (PowerShell):**
```powershell
.\setup.ps1
```

**macOS/Linux:**
```bash
chmod +x setup.sh && ./setup.sh
```

This automatically:
- ✅ Creates Python virtual environment
- ✅ Installs all dependencies
- ✅ Copies `.env.example` to `.env`
- ✅ Creates `.tmp/` directory

## Manual Setup (Alternative)

1. **Create and activate virtual environment:**
   ```powershell
   python -m venv .venv
   .\.venv\Scripts\Activate.ps1
   ```

2. **Install dependencies:**
   ```powershell
   pip install -r requirements.txt
   ```

3. **Set up environment:**
   ```powershell
   cp .env.example .env
   # Edit .env with your API keys
   ```

## Using the Agent

1. **Open in VS Code** - The custom agent is pre-configured
2. **Open Copilot Chat** - Press `Ctrl+Shift+I`
3. **Select your agent** - Choose "Fracktal Works F&A" from the agent dropdown
4. **Start working** - The agent knows the directives and scripts available

## VS Code Tasks (Optional)

Press `Ctrl+Shift+B` to run the default build task (Setup Agent Environment).

Other tasks available:
- **Setup Agent Environment** - Full one-command setup
- **Activate Virtual Environment** - Activate .venv
- **Install Requirements** - Install/update dependencies

## Structure

```
Fracktal Works F&A/
├── fracktal-works-f&a.code-workspace # ← Double-click to open VS Code!
├── setup.ps1 / setup.sh  # One-command setup scripts
├── AGENTS.md             # System prompt for AI agents
├── .env.example          # Template for API keys
├── requirements.txt      # Python dependencies
├── .github/agents/       # VS Code custom agent config
├── .vscode/              # VS Code settings & tasks
├── directives/           # What to do (SOPs)
└── execution/            # How to do it (scripts)
```

## Available Directives

| Directive | Description |
|-----------|-------------|
| `directives/fa_workflow_charts.md` | F&A workflow chart generation — 10 workflows covering AP, AR, Month-End Close, Expense Reimbursement, Payment Approval, Financial Reporting, Payroll, Bank Reconciliation, GST Compliance, Budget Planning |

## Quick Usage

```bash
# Generate ALL workflow charts (10 workflows)
python execution/fa_workflow_generator.py

# Generate specific workflows only
python execution/fa_workflow_generator.py --workflows "AP,AR,Payment Approval"

# Custom company name
python execution/fa_workflow_generator.py --company "Your Company Name"

# List available workflows
python execution/fa_workflow_generator.py --list-workflows
```

Output: `.tmp/FA_Workflow_Charts_<timestamp>.xlsx`

## Required API Keys

- **None required** for workflow chart generation (fully local)
- Optional: Google Sheets credentials for exporting charts to Sheets

## Google Credentials (if using Google Sheets)

If your agent uses Google Sheets scripts:
1. Place `credentials.json` in this folder
2. Run any sheet script once to generate `token.json`

---

*This workspace was generated from the [DOE Framework](https://github.com/vjvarada/DOE-Framework-Agentic-AI)*
